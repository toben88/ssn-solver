// Script to normalize the CSV data
const fs = require('fs');
...
for (let i = 1; i < csvData.length; i++) {
  ...
  // First 68 rows are already normalized
  if (i <= 68) {
  ...
  if (categoryId && !columns[1] && categoryNames[categoryId]) {
    columns[1] = categoryNames[categoryId];
  }
  
  // Format the long range and 75th year effects if they exist
  ...
  normalizedRows.push(columns.map(c => {
